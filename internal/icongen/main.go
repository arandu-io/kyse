...
func main() {
	...
	if err := write(out, src, icons); err != nil {
		log.Fatal(err)
	}
...
// write emits the generated Go and the upstream licence.
//
...
func write(out, src string, icons []icon) error {
	...
	//
	// The sweep skips _test.go, and that is not caution: icons_*.go matched
	// icons_test.go, and the first run of this generator deleted the tests. The
	// package still compiled, still passed, and reported "no tests to run" in a
	// line nobody reads.
...
// still says where its contents came from and under what licence.
func header() string {
...
package icons
`, phosphorVersion, weight, phosphorRepo, phosphorCommit)
}
...
var all = map[string]func(Props) template.HTML{
`)
	for _, i := range icons {
		fmt.Fprintf(&b, "\t%q: %s,\n", i.GoName, i.GoName)
	}
