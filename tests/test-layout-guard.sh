...
# ButtonTest.go compiles into the package as ordinary code and every test inside
# it is skipped -- no error, no warning, a green build with the suite switched
# off. Nothing else here can fail that quietly.
set -uo pipefail
